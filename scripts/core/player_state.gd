...
func setup(player_name: String, cards: Array, rng: RandomNumberGenerator, starting_health: int = STARTING_HEALTH) -> void:
	name = player_name
...
func draw_cards(count: int, rng: RandomNumberGenerator) -> void:
	var open_slots: int = max(MAX_HAND_SIZE - hand.size(), 0)
